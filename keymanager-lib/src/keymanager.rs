...
fn init_kdf(req: &InitRequest, ctx: &mut RpcContext) -> Result<SignedInitResponse> {
    let policy_checksum = Policy::global().init(ctx, &req.policy).with_context(|| format!("policy global init {:?}", &req.policy))?;
    Kdf::global().init(req, ctx, policy_checksum).with_context(|| "kdf global init")
}
